...
class Seminar(Module):
    ...
    def __str__(self):
        return "{0:s} under the topic: {1:s}".format(self.title, self.topic)
...
class Thesis(Module):
    ...
    def __str__(self):
        return "{0:s} on the topic: {1:s} in the research group {2:s}".format(self.title, self.topic, self.research_group)
